	      ...
	      <div class="pokemon-${effectiveness} tooltip">
              ...
              .map(
                (type, counter) => `
	              ...
	              ${(counter + 1) % 3 === 0 || counter + 1 === typeEffectivenesses[effectiveness].length ? '</div>' : ''}
	          `
              )
              .join('')}
...
function _createEnemyDiv() {
  console.trace('_createEnemyDiv 호출');
  const divId = 'enemies';
...
function _createAlliesDiv() {
  console.trace('_createAlliesDiv 호출');
  const divId = 'allies';
  const div = document.createElement('div');
  div.className = `${divId}-team`;
  div.id = divId;
